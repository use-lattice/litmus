...
import { filterProviderConfigs, getProviderIdAndLabel } from '../filterProviders';
...
export async function resolveConfigs(
  ...
  if (!hasConfigFile && !hasPrompts && !hasProviders && !isCI()) {
    ...
    logger.warn(dedent`
      No promptfooconfig found

      Searched in ${process.cwd()} for promptfooconfig.{${extList}}

      Try running with:

      ${promptfooCommand('')} eval -c path/to/promptfooconfig.yaml

      Or create a config with:

      ${promptfooCommand('init')}
    `);
    process.exit(1);
  }
  if (!hasPrompts) {
  ...
  const filteredProviderConfigs = filterProviderConfigs(
    Array.isArray(cliFilteredProviderConfigs)

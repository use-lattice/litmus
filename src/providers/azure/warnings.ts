...
export function maybeEmitAzureOpenAiWarning(testSuite: TestSuite, tests: TestCase[]) {
  ...
  if (
    ...
    if (modelGradedAsserts.length > 0) {
      ...
      logger.warn(
        `You are using model-graded assertions of types ${assertTypes} while testing an Azure provider. You may need to override these to use your Azure deployment. To learn more, see https://promptfoo.dev/docs/providers/azure/#model-graded-tests`,
      );
      return true;
    }
  }

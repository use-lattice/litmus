...
/**
 * No-op progress bar manager — CLI progress removed.
 * Preserves the public interface so the rest of evaluator.ts needs no changes.
 */
export class ProgressBarManager {
  constructor(_isWebUI: boolean) {}

  installLogInterceptor(): void {}
  removeLogInterceptor(): void {}

  async initialize(
    _runEvalOptions: RunEvalOptions[],
    _concurrency: number,
    _compareRowsCount: number,
  ): Promise<void> {}

  updateProgress(
    _index: number,
    _evalStep: RunEvalOptions | undefined,
    _phase: 'serial' | 'concurrent' = 'concurrent',
    _metrics?: PromptMetrics,
  ): void {}

  updateComparisonProgress(_prompt: string): void {}
  updateTotalCount(_additionalCount: number): void {}
  complete(): void {}
  stop(): void {}
}
...
class Evaluator {
  ...
  private async _runEvaluation(): Promise<Eval> {
    ...
    if (options.generateSuggestions) {
      ...
      if (numAdded < 1) {
        logger.info('No prompts selected. Aborting.');
        process.exitCode = 1;
    ...
    if (concurrency > 1) {
      ...
      if (usesConversation) {
        logger.info(`Setting concurrency to 1 because the _conversation variable is used.`);
        concurrency = 1;

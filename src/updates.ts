...
export async function checkForUpdates(): Promise<boolean> {
  ...
  if (semverGt(latestVersion, VERSION)) {
    ...
    logger.info(
      `\n${border}
Warning: The current version of promptfoo ${VERSION} is lower than the latest available version ${latestVersion}.
...
export async function checkModelAuditUpdates(): Promise<boolean> {
  ...
  if (semverGt(latestVersion, currentVersion)) {
    ...
    logger.info(
      `\n${border}
Warning: The current version of modelaudit ${currentVersion} is lower than the latest available version ${latestVersion}.

Please run 'pip install --upgrade modelaudit' to update.
${border}\n`,
    );
    return true;
  }

...
export async function promptForEmailUnverified(): Promise<{ emailNeedsValidation: boolean }> {
  ...
  if (!email) {
    ...
    // In non-interactive (library) mode, require email via env var
    const envEmail = getEnvString('PROMPTFOO_EMAIL');
    if (!envEmail) {
      throw new Error(
        'Email verification required for red team scans. ' +
          'Set the PROMPTFOO_EMAIL environment variable to your work email address.',
      );
    }
    email = envEmail;
    setUserEmail(email);

...
export async function doRedteamRun(options: RedteamRunOptions): Promise<Eval | undefined> {
  ...
  if (!evalResult?.shared) {
    if (options.liveRedteamConfig) {
      logger.info(
        `To view the results, click the View Report button or run ${promptfooCommand('redteam report')} on the command line.`,
      );
    } else {
      logger.info(`To view the results, run ${promptfooCommand('redteam report')}`);
    }
  }

...
function generateReport(
  ...
  lines.push(
    `${pad('#', 5)} ${pad('Type', 10)} ${pad('ID', 40)} ${pad('Requested', 12)} ${pad('Generated', 12)} Status`,
  );
  ...
  Object.entries(pluginResults)
    ...
    .forEach(([displayId, { requested, generated }]) => {
      lines.push(
        `${pad(rowIndex++, 5)} ${pad('Plugin', 10)} ${pad(displayId, 40)} ${pad(requested, 12)} ${pad(generated, 12)} ${getStatus(requested, generated)}`,
      );
    });
  ...
  Object.entries(strategyResults)
    ...
    .forEach(([id, { requested, generated }]) => {
      lines.push(
        `${pad(rowIndex++, 5)} ${pad('Strategy', 10)} ${pad(id, 40)} ${pad(requested, 12)} ${pad(generated, 12)} ${getStatus(requested, generated)}`,
      );
    });
...
export async function synthesize({
  ...
  logger.info(
    `Test Generation Summary:` +
      ...
      `\n• Plugin tests: ${totalPluginTests}` +
      `\n• Plugins: ${plugins.length}` +
      `\n• Strategies: ${effectiveStrategyCount}` +
  ...
  // Progress tracking (no CLI progress bar — just log messages)
  const isWebUI = Boolean(cliState.webUI);

  const showProgressBar = false as boolean; // CLI progress bar removed; use logger instead
  let progressBar: {
    update: (o: any) => void;
  ...
  if (showProgressBar) {
    // Dead code path: would create a real progress bar if enabled
    progressBar = { update: () => {}, increment: () => {}, start: () => {}, stop: () => {} };
  }

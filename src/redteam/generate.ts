...
function handleFailedPlugins(failedPlugins: FailedPluginInfo[], strict: boolean): void {
  ...
  const warningMessage = dedent`
    ${'⚠️  Warning:'} Test case generation failed for ${failedPlugins.length} plugin(s):
    ${pluginList.join('\n')}
  ...
  logger.warn(
    `Continuing with partial results. Use --strict flag to fail on plugin generation errors.`,
  );
}
...
export async function doGenerateRedteam(
  ...
  if (!probeLimitResult.withinLimit) {
    logger.error(dedent`
      ${'Monthly probe limit reached'}

      You've used ${probeLimitResult.used.toLocaleString()} of your ${MONTHLY_PROBE_LIMIT.toLocaleString()} free monthly probes.

      To continue, please log in to Promptfoo Cloud:

        ${'promptfoo auth login'}

      For enterprise plans, contact ${'inquiries@promptfoo.dev'}
    `);
    process.exitCode = 1;
    return null;
  }
  ...
  if (configPath) {
    ...
    if (redteamConfig && resolved.testSuite.tests && resolved.testSuite.tests.length > 0) {
      logger.warn(
        dedent`
            Warning: Found both 'tests' section and 'redteam' configuration in your config file.
    ...
    logger.info(
      `\nCan't generate without configuration - run ${promptfooCommand('redteam init')} first`,
    );
    return null;
  }
  ...
  try {
    ...
    if (options.output && options.output.endsWith('.burp')) {
      ...
      logger.info(`Wrote ${redteamTests.length} test cases to ${options.output}`);
      ...
      logger.info('\n' + `Run ${`${command}`} to run the red team!`);
    } else {
      const author = getAuthor();
      const userEmail = getUserEmail();
      const cloudHost = userEmail ? cloudConfig.getApiHost() : null;

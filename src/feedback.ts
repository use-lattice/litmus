...
export async function sendFeedback(feedback: string) {
  ...
  try {
    ...
    if (resp.ok) {
      ...
      logger.info('Failed to send feedback. Please try again or open an issue on GitHub.');
    }
  } catch {
...
export async function gatherFeedback(message?: string) {
  ...
  try {
    ...
    const feedbackText = await promptUser(
      dedent`
      Share your thoughts, bug reports, or feature requests:
      > `,
    );
    ...
    if (userEmail) {
      ...
      const contactResponse = await promptUser(
        "Email address (optional, if you'd like a response): ",
      );

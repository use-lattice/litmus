...
import { doGenerateRedteam } from '../../src/redteam/generate';
...
vi.mock('../../src/redteam/generate');
vi.mock('../../src/util/verboseToggle', () => ({
  initVerboseToggle: vi.fn(),
}));
vi.mock('../../src/doEval', async (importOriginal) => {
  return {
...
describe('doRedteamRun', () => {
  ...
  it('should locate the out file in the same directory as the config file if output is not specified', async () => {
    ...
    // FakeDataFactory was removed; use a fixed path
    const dirPath = '/tmp/fake/redteam';

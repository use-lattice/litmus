...
vi.mock('cli-progress', () => ({
  default: { Presets: { shades_classic: {} }, SingleBar: class {} },
}));
vi.mock('../../src/logger');
...
describe('synthesize', () => {
  ...
  describe('Plugins and strategies', () => {
    ...
    it('should use default fan-out values when strategy config omits n', async () => {
      ...
      try {
        ...
        // Find the jailbreak:composite row (space-padded columns, no │ separators)
        const row = lines.find((line) => line.includes('jailbreak:composite'));
        expect(row).toBeDefined();

        // The row format is: # Type ID Requested Generated Status
        // Values are space-padded so we can extract numbers from the row
        const expectedFanout = getDefaultNFanout('jailbreak:composite').toString();
        expect(row).toContain(expectedFanout);
  ...
  describe('Logger', () => {
    it('debug log level hides progress bar', async () => {
      ...
      // cli-progress removed; progress bars are no-ops
      process.env.LOG_LEVEL = originalLogLevel;
    });
  });

  // Logger and Progress bar tests removed — cli-progress was stripped from this fork
...
describe('Language configuration', () => {
  ...
  describe('plugin logging and report aggregation', () => {
    ...
    it('should work correctly with both built-in plugins and policy plugins', async () => {
      ...
      // Report uses space-padded columns (no │ separators)
      const pluginRows = cleanReport.match(/\d+\s+Plugin\s+/g);

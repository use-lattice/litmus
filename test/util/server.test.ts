...
describe('Server Utilities', () => {
  ...
  describe('openBrowser', () => {
    it('should log URL with default port when BrowserBehavior.OPEN', async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        expect.stringContaining(`http://localhost:${getDefaultPort()}`),
      );
    });

    it('should log report URL when BrowserBehavior.OPEN_TO_REPORT', async () => {
      await openBrowser(BrowserBehavior.OPEN_TO_REPORT);

      expect(logger.info).toHaveBeenCalledWith(
        expect.stringContaining(`http://localhost:${getDefaultPort()}/report`),
      );
    });

    it('should log redteam setup URL when BrowserBehavior.OPEN_TO_REDTEAM_CREATE', async () => {
      await openBrowser(BrowserBehavior.OPEN_TO_REDTEAM_CREATE);

      expect(logger.info).toHaveBeenCalledWith(
        expect.stringContaining(`http://localhost:${getDefaultPort()}/redteam/setup`),
      );
    });

    it('should log eval setup URL when BrowserBehavior.OPEN_TO_EVAL_SETUP', async () => {
      await openBrowser(BrowserBehavior.OPEN_TO_EVAL_SETUP);

      expect(logger.info).toHaveBeenCalledWith(
        expect.stringContaining(`http://localhost:${getDefaultPort()}/setup`),
      );
    });

    it('should not log when BrowserBehavior.SKIP', async () => {
      await openBrowser(BrowserBehavior.SKIP);

      expect(logger.info).not.toHaveBeenCalled();
    });

    it('should ask user before logging URL when BrowserBehavior.ASK', async () => {
    ...
    it('should not log URL when user answers no to ASK prompt', async () => {
    ...
    it('should use custom port when provided', async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        expect.stringContaining(`http://localhost:${customPort}`),
      );
    });
  });
